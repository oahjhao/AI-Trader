...
def convert_a_stock_to_jsonl(
    csv_path: str = "A_stock_daily.csv",
        ...
        for stock_code, group_df in grouped:
            ...
            for idx, row in group_df.iterrows():
                ...
                time_series[date_formatted] = {
                    ...
                    "A1. MA_5": str(row["MA_5"]),
                    "A2. MA_10": str(row["MA_10"]),
                    "A3. MA_20": str(row["MA_20"]),
                    "A4. MA_60": str(row["MA_60"]),
                    "B1. EMA_5": str(row["EMA_5"]),
                    "B2. EMA_10": str(row["EMA_10"]),
                    "B3. EMA_20": str(row["EMA_20"]),
                    "B4. EMA_60": str(row["EMA_60"]),
                    "C1. RSI": str(row["RSI"]),
                    "C2. DIF": str(row["DIF"]),
                    "C3. DEA": str(row["DEA"]),
                    "C4. ATR": str(row["ATR"]),
...
if __name__ == "__main__":
    ...
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='A-Share数据转JSONL格式')
    parser.add_argument('--date-suffix', type=str, default='', help='日期后缀 (YYYYMMDD)')
    parser.add_argument('--csv-path', type=str, default='A_stock_daily.csv', help='CSV文件路径')
    parser.add_argument('--output-path', type=str, default='merged.jsonl', help='输出JSONL文件路径')
